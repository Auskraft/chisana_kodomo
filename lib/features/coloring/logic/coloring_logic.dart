...
enum ColoringMode {
  /// Раскрасить: контурная картинка + набор инструментов ([PaintTool]).
  fill,
...
/// Инструмент в режиме «Раскрасить» (растровая контурная картинка). Все, кроме
/// [fill], кладут мазки ВНУТРИ области, по которой ведёт малыш («удержание в
/// контуре» через маску заливки) — даже неровные движения остаются аккуратными.
enum PaintTool {
  /// Заливка: тап по области → залить её целиком выбранным цветом.
  fill,

  /// Карандаш: тонкий плотный штрих.
  pencil,

  /// Маркер: толстый яркий штрих.
  marker,

  /// Акварель: полупрозрачная, копится слоями, мягкие края.
  watercolor,

  /// Гуашь: плотная насыщенная краска.
  gouache,
}
